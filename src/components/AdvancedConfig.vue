    ...
    <el-form label-width="120px">
      ...
      <template v-if="!isPieChart">
        <el-form-item label="堆叠显示">
          <el-switch v-model="config.stack" @change="updateAdvance" />
        </el-form-item>

        <el-form-item label="显示数据标签">
          <el-switch v-model="config.showLabel" @change="updateAdvance" />
        </el-form-item>

        <el-form-item label="标签位置" v-if="config.showLabel">
          <el-select v-model="config.labelPosition" @change="updateAdvance">
      ...
      <template v-if="isPieChart">
        <el-form-item label="饼图半径">
          <el-input v-model="config.radius" placeholder="例如: 50% 或 ['40%', '70%']" @change="updateAdvance" />
        </el-form-item>

        <el-form-item label="玫瑰图模式">
          <el-select v-model="config.roseType" @change="updateAdvance">
        ...
        <el-form-item label="显示百分比">
          <el-switch v-model="config.showPercent" @change="updateAdvance" />
        </el-form-item>
      </template>
      ...
      <template v-if="chartType === 'bar'">
        <el-form-item label="柱条宽度">
          <el-input v-model="config.barWidth" placeholder="例如: 20 或 30%" @change="updateAdvance" />
        </el-form-item>

        <el-form-item label="柱条圆角">
          <el-slider v-model="config.barBorderRadius" :min="0" :max="20" @change="updateAdvance" />
        </el-form-item>
      </template>
      ...
      <template v-if="chartType === 'scatter'">
        <el-form-item label="点大小">
          <el-slider v-model="config.symbolSize" :min="5" :max="50" @change="updateAdvance" />
        </el-form-item>

        <el-form-item label="显示涟漪效果">
          <el-switch v-model="config.rippleEffect" @change="updateAdvance" />
        </el-form-item>
      </template>
    </el-form>
  </div>
